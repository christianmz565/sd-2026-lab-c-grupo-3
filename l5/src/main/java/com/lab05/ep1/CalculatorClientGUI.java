...
public class CalculatorClientGUI extends JFrame {
  ...
  private void doCalculate() {
    ...
    new SwingWorker<RCPResult, Void>() {
      @Override
      protected RCPResult doInBackground() throws Exception {
      ...
      protected void done() {
        try {
          ...
          appendHistory(
            formatLine(OPS[op][0], OPS[op][1], a, b, res.value(), res.ms())
          );
        } catch (Exception ex) {
          resultLabel.setForeground(DANGER);
  ...
  private void doCalculateAll() {
    ...
    new SwingWorker<Void, Void>() {
      ...
      protected Void doInBackground() throws Exception {
        appendHistory(
          "── Todas las operaciones (A=" + fa + ", B=" + fb + ") ──"
        );
        for (int i = 0; i < OPS.length; i++) {
          try {
            RCPResult res = invoke(i, fa, fb);
            appendHistory(
              formatLine(OPS[i][0], OPS[i][1], fa, fb, res.value(), res.ms())
            );
            if (i == selectedOp) SwingUtilities.invokeLater(() ->
              showResult(res.value())
            );
          } catch (Exception ex) {
