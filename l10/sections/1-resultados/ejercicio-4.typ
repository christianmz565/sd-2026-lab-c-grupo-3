...
== Proceso Técnico de Simulación y Verificación

El script init-replication.sh y el archivo README.md documentan los comandos para simular este failover en el entorno local de Docker:

=== Paso 1: Caída de Lima-DB
Se detiene el contenedor del nodo principal para simular la pérdida de conectividad o falla de hardware:
```bash
docker compose -f docker-compose.multi.yml stop lima-db
```

=== Paso 2: Promoción de Bogotá
Se ejecuta la promoción de la réplica de Bogotá a nodo primario:
```bash
docker compose -f docker-compose.multi.yml exec bogota-db psql -U postgres -c "SELECT pg_promote();"
```

=== Paso 3: Verificación del Estado de Recuperación
Se comprueba si Bogotá sigue en modo de recuperación (lectura):
```bash
docker compose -f docker-compose.multi.yml exec bogota-db psql -U postgres -c "SELECT pg_is_in_recovery();"
```
Resultado esperado: Devuelve f (false), confirmando que ya no es standby y acepta escrituras.

=== Paso 4: Inserción de Datos en el Nuevo Primario
Para confirmar que el nodo de Bogotá ahora acepta escrituras, se inserta un registro de prueba de inventario:
```bash
docker compose -f docker-compose.multi.yml exec bogota-db psql -U postgres -c "
INSERT INTO inventarios (centro, producto, cantidad) 
VALUES ('Bogota', 'Post-failover-package', 999);
"
```
El comando se ejecuta con éxito, validando la continuidad operativa desde Bogotá.
