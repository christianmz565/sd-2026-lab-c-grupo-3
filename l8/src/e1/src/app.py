...
app = FastAPI(
    ...
    description="Coordinador HTTP del protocolo Two-Phase Commit sobre 3 nodos PostgreSQL (Arequipa, Lima, Cusco). Caso de estudio Lab 08 - Sistemas Distribuidos.",
    version="0.1.0",
    lifespan=lifespan,
)
app.add_middleware(
...
def health() -> HealthResponse:
    ...
    return HealthResponse(
        arequipa=status("arequipa"), lima=status("lima"), cusco=status("cusco")
    )
...
def inventario() -> InventarioResponse:
    ...
    for nodo in ("arequipa", "lima", "cusco"):
        ...
        for it in items:
            rows.append(
                InventarioRow(almacen=nodo, producto=it["producto"], stock=it["stock"])
            )
