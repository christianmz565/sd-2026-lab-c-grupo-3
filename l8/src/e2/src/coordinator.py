...
class TwoPhaseCommitCoordinator:
    ...
    def transferir(
        ...
        self.log.append(
            ...
            "START",
            None,
            f"{ciudad_origen}({cuenta_origen})->{ciudad_destino}({cuenta_destino}) S/ {monto:.2f}",
        )
    ...
    def _validate_prepared_connections(self, prepared: _PreparedTxn) -> None:
        ...
        for name, conn in [
            ...
            try:
                ...
                raise TransferError(
                    f"Sucursal {name} no responde tras el retraso: {clean_err}"
                ) from e
    ...
    def _phase_one(
        ...
        try:
            ...
            clean_err = db.simplify_db_error(e)
            self.log.append(
                txn_id, "FAILED", ciudad_origen, f"error de conexión: {clean_err}"
            )
            raise TransferError(
                f"No se pudo conectar a la sucursal {ciudad_origen}: {clean_err}"
            ) from e
        ...
        try:
            ...
            self._rollback_all(opened)
            raise TransferError(
                f"Fallo de BD en sucursal {ciudad_origen}: {clean_err}"
            ) from e
        try:
            ...
            clean_err = db.simplify_db_error(e)
            self.log.append(
                txn_id, "FAILED", ciudad_destino, f"error de conexión: {clean_err}"
            )
            self._rollback_all(opened)
            raise TransferError(
                f"No se pudo conectar a la sucursal {ciudad_destino}: {clean_err}"
            ) from e
        ...
        try:
            ...
            self.log.append(
                txn_id, "FAILED", ciudad_destino, f"rechazo en PREPARE: {e}"
            )
            self._rollback_all(opened)
            raise TransferError(str(e)) from e
        except (pg_errors.Error, psycopg.OperationalError) as e:
            ...
            self._rollback_all(opened)
            raise TransferError(
                f"Fallo de BD en sucursal {ciudad_destino}: {clean_err}"
            ) from e
        return _PreparedTxn(
        ...
        for i, (ciudad, conn, key) in enumerate(plan):
            ...
            try:
                with psycopg.connect(
                    db.get_sucursal(ciudad).dsn, autocommit=True
                ) as c2:
                    saldo = db.read_saldo(
                        c2, cuenta_origen if key == "origen" else cuenta_destino
                    )
                    if key == "origen":
    ...
    def _phase_two_rollback(
        self, prepared: _PreparedTxn, ciudad_origen: str, ciudad_destino: str
    ) -> None:
