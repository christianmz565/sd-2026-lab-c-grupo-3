...
const SELECTIVE_FIELDS_QUERY = JSON.stringify({
  query: `{
    books {
      id
      title
...
export default function () {
  ...
  const res = http.post(`${BASE_URL}/graphql`, SELECTIVE_FIELDS_QUERY, params);

  latency.add(res.timings.duration);
  check(res, {
    'status is 200': (r) => r.status === 200,
    'response has selective fields': (r) => {
      try {
        const body = JSON.parse(r.body);
        if (!body.data || !Array.isArray(body.data.books)) return false;
        const book = body.data.books[0];
        return book && book.title && book.author && !book.description;
      } catch (e) {
        return false;
      }
    },
...
export function handleSummary(data) {
  ...
  return {
    '/tmp/opencode/graphql_selective_fields_results.json': JSON.stringify({
      test: 'GraphQL SELECTIVE Fields (title+author only)',
