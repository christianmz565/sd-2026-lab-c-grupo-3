...
const CREATE_BOOK_MUTATION = (id) => JSON.stringify({
  query: `mutation {
    createBook(input: {
      title: "Test Book ${id}"
...
export default function () {
  ...
  const res = http.post(`${BASE_URL}/graphql`, CREATE_BOOK_MUTATION(uniqueId), params);

  latency.add(res.timings.duration);
  check(res, {
    'status is 200': (r) => r.status === 200,
    'response has book data': (r) => {
      try {
        const body = JSON.parse(r.body);
        return body.data && body.data.createBook && body.data.createBook.id;
      } catch (e) {
        return false;
      }
    },
...
export function handleSummary(data) {
  ...
  return {
    '/tmp/opencode/graphql_write_results.json': JSON.stringify({
      test: 'GraphQL WRITE (Create Book)',
